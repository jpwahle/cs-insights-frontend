  ...
  (function (data) {
    ...
    function init_forms(topicID, lambdaID, visID) {
      ...
      lambdaLabel.innerHTML =
        "&#955 = <span id='" + lambdaID + "-value'>" + vis_state.lambda + '</span>';
    ...
    function topic_on(circle) {
      ...
      d3.select('#bar-freqs')
        ...
        .style('font-size', '16px')
        .text(
          'Top-' + R + ' Most Relevant Terms for Topic ' + topics + ' (' + Freq + '% of tokens)'
        );
    ...
    function topic_off(circle) {
      ...
      var title = d3.selectAll('.bubble-tool').text('Top-' + R + ' Most Salient Terms');
      title.append('tspan').attr('baseline-shift', 'super').attr('font-size', 12).text(1);
